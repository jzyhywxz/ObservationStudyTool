...
namespace ObservationStudyTool.Helpers
{
    ...
    public class CsvSerializer<T> where T : class, new()
    {
        ...
        {
            // Created; TaskDuration; TaskType; TaskDescription; TaskIdentifier
            return string.Format(
                    "Created{0}TaskDuration{0}TaskType{0}TaskDescription{0}TaskIdentifier{0}",
                    Separator);

            // Created; TaskDuration; TaskDescription; TaskNumber; Application; ActivityCategory; ReasonForCs
            /*return String.Format(
                    "Created{0}TaskDuration{0}TaskDescription{0}TaskNumber{0}Application{0}ActivityCategory{0}ReasonForCs{0}",
                    Separator);*/
        ...
        public void Serialize(Stream stream, IList<T> data, char separator)
        {
            Separator = separator;
            var sb = new StringBuilder();
            //var values = new List<string>();

            sb.AppendLine("sep=;"); // automatic recognition of semi-colon-separated file
    ...
    {
        public InvalidCsvFormatException(string message, Exception innerException)
